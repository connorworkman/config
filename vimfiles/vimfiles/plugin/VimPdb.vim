...
function! PdbMapKeyboard()
	...
	map <buffer> <silent> <Leader><F5> :call PdbStartDebug(1, [])<CR>
	" Start debug and don't pause immediately.
	map <buffer> <silent> <Leader><C-F5> :call PdbStartDebug(0, [])<CR>
	map <buffer> <silent> <Leader><C-S-F5> :call PdbStartDebugWithArguments()<CR>
	map <buffer> <silent> <Leader><S-F5> :call PdbStopDebug()<CR>
	map <buffer> <silent> <Leader><C-A-S-F5> :call PdbRestartDebug()<CR>
	...
	map <buffer> <silent> <Leader><F7> :call PdbStepInto()<CR>
	map <buffer> <silent> <Leader><F8> :call PdbStepOver()<CR>
	map <buffer> <silent> <Leader><C-F8> :call PdbContinueUntilReturn()<CR>

	map <buffer> <silent> <Leader><F9> :call PdbMoveUpInStackFrame()<CR>
	map <buffer> <silent> <Leader><F10> :call PdbMoveDownInStackFrame()<CR>

	map <buffer> <silent> <Leader><F6> :call PdbSetFocusToCurrentDebugLine()<CR>
	map <buffer> <silent> <Leader><C-F6> :call PdbJumpToCurrentLine()<CR>

	map <buffer> <silent> <Leader><F2> :call PdbToggleBreakpointOnCurrentLine()<CR>
	map <buffer> <silent> <Leader><C-F2> :call PdbToggleConditionalBreakpointOnCurrentLine()<CR>
	map <buffer> <silent> <Leader><S-F2> :call PdbToggleTemporaryBreakpointOnCurrentLine()<CR>
	map <buffer> <silent> <Leader><C-S-F2> :call PdbClearAllBreakpointsInCurrentFile()<CR>
	map <buffer> <silent> <Leader><C-A-S-F2> :call PdbClearAllBreakpoints()<CR>

	map <buffer> <silent> <Leader><F11> :call PdbPrintBreakpointConditionOnCurrentLine()<CR>

	map <buffer> <silent> <Leader><F4> :call PdbEvalCurrentWord()<CR>
	map <buffer> <silent> <Leader><C-F4> :call PdbEvalCurrentWORD()<CR>

	map <buffer> <silent> <Leader><F3> :call PdbEvalExpression()<CR>
	map <buffer> <silent> <Leader><C-F3> :call PdbExecStatement()<CR>

	map <buffer> <silent> <Leader><F12> :call PdbPrintStackTrace()<CR>
endfunction
