...
plugins=(screen zshmarks tmux tmuxinator gpg-agent ssh-agent archlinux fancy-ctrl-z adb command-not-found systemd screen man nmap colorize extract catimg copydir copyfile compleat cp dirpersist fasd mosh tmux-cssh wd url-tools zsh-navigation-tools z safe-paste themes rsync celery repo pass pip brew web-search thefuck common-aliases gnu-utils git-remote-branch git gitignore git-extras github brew iwhois zsh-autosuggestions history-search-multi-word)
